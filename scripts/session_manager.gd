...
# Keep the wire value compatible with the previous Steam protocol. The package
...
var _steam_transport = null
var _lan_transport = null
var _lan_discovery = null
var _session_active = false
var _session_id = ""
var _connection_by_peer_key = {}
...
func _ready() -> void:
	...
	call_deferred("_bind_transports")
	call_deferred("_check_launch_join_args")
	_update_online_session_runtime_flag()


func _bind_transports() -> void:
	var parent = get_parent()
	if parent == null:
		return
	_steam_transport = parent.get_node_or_null("BrotatoOnlineSteamTransport")
	_lan_transport = parent.get_node_or_null("BrotatoOnlineLanTransport")
	_lan_discovery = parent.get_node_or_null("BrotatoOnlineLanDiscovery")
	if _steam_transport != null:
		_steam = _steam_transport.get_steam()
		_steam_ready = _steam_transport.is_available()
		_connect_steam_transport_signals()
	else:
		_setup_steam()
		_connect_steam_signals()
	if _lan_transport != null:
		_connect_transport_signal(_lan_transport, "peer_connected", "_on_transport_peer_connected")
...
func _connect_steam_transport_signals() -> void:
	for pair in [
		...
		["lobby_join_requested", "_on_lobby_join_requested"],
		["rich_presence_join_requested", "_on_rich_presence_join_requested"],
		["join_game_requested", "_on_join_game_requested"],
		["network_messages_session_request", "_on_network_messages_session_request"],
		["network_messages_session_failed", "_on_network_messages_session_failed"]
	]:
		_connect_transport_signal(_steam_transport, pair[0], pair[1])


func _connect_transport_signal(transport: Node, signal_name: String, method_name: String) -> void:
	if transport != null and transport.has_signal(signal_name) and not transport.is_connected(signal_name, self, method_name):
		transport.connect(signal_name, self, method_name)


func join_lan(address: String, port: int = DEFAULT_LAN_PORT) -> void:
	address = address.strip_edges()
	if address == "" or port <= 0 or port > 65535:
	...
	if _lan_transport == null or int(_lan_transport.start_client(address, port)) != OK:
		_pending_lan_join = false
		_online_role = "none"
		_show_join_failure(_ui_text("join_failed_lan_unavailable"))
	_update_online_session_runtime_flag()


func _on_transport_peer_connected(transport: Node, peer) -> void:
	if transport != _lan_transport:
		return
	if _is_game_host() and _session_active:
		var peer_key = transport.get_peer_key(peer)
		var accepted = _register_remote_connection(peer_key, transport, peer)
		var welcome = {
			"msg_type": "transport_welcome",
			"accepted": accepted,
			"protocol_version": LAN_PROTOCOL_VERSION,
			"mod_version": NETWORK_PROTOCOL_VERSION,
			"session_id": _get_wire_session_id(),
			"assigned_peer_key": peer_key,
			"host_peer_key": _self_steam_id,
			"member_count": get_session_member_count(),
			"member_limit": MAX_LOBBY_MEMBERS,
			"state": get_lobby_state(),
			"reason": "room_full" if not accepted else ""
		}
		transport.send_packet(peer, to_json(welcome).to_utf8(), P2P_CHANNEL_MENU, true)
		if not accepted:
		...
		# A LAN client becomes active only after the host's welcome supplies the
		# shared session id and its assigned opaque peer key.
		_peer_key_by_connection[_connection_lookup_key(transport, peer)] = "__pending_host__"


func _on_transport_peer_disconnected(transport: Node, peer) -> void:
	var lookup_key = _connection_lookup_key(transport, peer)
	var peer_key = str(_peer_key_by_connection.get(lookup_key, ""))
	_peer_key_by_connection.erase(lookup_key)
...
func _on_transport_connection_failed(transport: Node, reason) -> void:
	if transport == _lan_transport and _pending_lan_join:
		_pending_lan_join = false
		_online_role = "none"
		_client_join_request_started_msec = 0
		_update_online_session_runtime_flag()
		_show_join_failure(_ui_text("join_failed_lan_unavailable") + " (" + str(reason) + ")")


func _on_transport_packet_received(transport: Node, peer, data: PoolByteArray, channel: int) -> void:
	if data.empty():
		return
	var parsed = parse_json(data.get_string_from_utf8())
	if typeof(parsed) == TYPE_DICTIONARY:
		var transport_msg_type = str(parsed.get("msg_type", ""))
		if transport_msg_type == "transport_welcome":
			_handle_transport_welcome(transport, peer, parsed)
			return
		if transport_msg_type == "transport_session_id":
			_handle_transport_session_id(transport, parsed)
			return
	var lookup_key = _connection_lookup_key(transport, peer)
	var peer_key = str(_peer_key_by_connection.get(lookup_key, ""))
	# LAN packets already carry the opaque sender id assigned during the welcome
	# handshake. Cross-check it against a connection registered on this transport
	# instead of silently dropping valid client menu actions when ENet peer metadata
	# is temporarily stale or mismatched.
	if transport == _lan_transport and typeof(parsed) == TYPE_DICTIONARY:
		var declared_peer_key = str(parsed.get("sender_steam_id", ""))
		var declared_connection = _connection_by_peer_key.get(declared_peer_key, {})
		if declared_peer_key != "" and declared_peer_key != "0" and typeof(declared_connection) == TYPE_DICTIONARY and declared_connection.get("transport", null) == transport:
			peer_key = declared_peer_key
			_peer_key_by_connection[lookup_key] = declared_peer_key
	if peer_key == "" or peer_key == "__pending_host__":
		return
	_handle_raw_p2p_packet({"sender": peer_key, "bytes": data, "channel": channel})


func _handle_transport_welcome(transport: Node, peer, welcome: Dictionary) -> void:
	if transport != _lan_transport or _is_game_host():
		return
	if not bool(welcome.get("accepted", false)):
		_pending_lan_join = false
		transport.stop()
		_show_join_failure(_ui_text("join_failed_room_full"))
		return
	if int(welcome.get("protocol_version", 0)) != LAN_PROTOCOL_VERSION or str(welcome.get("mod_version", "")) != NETWORK_PROTOCOL_VERSION:
		_pending_lan_join = false
		transport.stop()
		_show_join_failure(_ui_text("join_failed_protocol_mismatch"))
		return
	_session_id = str(welcome.get("session_id", ""))
	_self_steam_id = str(welcome.get("assigned_peer_key", ""))
	_game_host_steam_id = str(welcome.get("host_peer_key", ""))
	if _session_id == "" or _self_steam_id == "" or _game_host_steam_id == "":
		transport.stop()
		return
	...
	_connection_by_peer_key[_game_host_steam_id] = {"transport": transport, "peer": peer, "peer_key": _game_host_steam_id}
	_update_online_session_runtime_flag()
	_start_client_hello_retry()
	_send_client_hello_to_host()


func _handle_transport_session_id(transport: Node, message: Dictionary) -> void:
	if transport != _lan_transport or _is_game_host():
		return
	if int(message.get("protocol_version", 0)) != LAN_PROTOCOL_VERSION:
		return
	if str(message.get("mod_version", "")) != NETWORK_PROTOCOL_VERSION:
		return
	var next_session_id = str(message.get("session_id", ""))
	if next_session_id == "":
		return
	_session_id = next_session_id


func _broadcast_transport_session_id() -> void:
	if not _is_game_host() or _lan_transport == null:
		return
	var wire_session_id = _get_wire_session_id()
	if wire_session_id == "":
		return
	var payload = to_json({
		"msg_type": "transport_session_id",
		"protocol_version": LAN_PROTOCOL_VERSION,
		"mod_version": NETWORK_PROTOCOL_VERSION,
		"session_id": wire_session_id
	}).to_utf8()
	for peer_key in _connection_by_peer_key.keys():
		var connection = _connection_by_peer_key.get(peer_key, {})
		if typeof(connection) != TYPE_DICTIONARY or connection.empty():
			continue
		if connection.get("transport", null) != _lan_transport:
			continue
		_lan_transport.send_packet(connection.get("peer", null), payload, P2P_CHANNEL_MENU, true)
...
func _register_remote_connection(peer_key: String, transport: Node, peer) -> bool:
	...
	if _connection_by_peer_key.has(peer_key):
		_connection_by_peer_key[peer_key] = {"transport": transport, "peer": peer, "peer_key": peer_key}
		_peer_key_by_connection[_connection_lookup_key(transport, peer)] = peer_key
		_refresh_player_connection(peer_key)
		return true

	var takeover_key = ""
	for existing_key_value in _session_remote_peer_keys:
		var existing_key = str(existing_key_value)
		if not _connection_by_peer_key.has(existing_key):
			takeover_key = existing_key
			break
	if takeover_key != "":
		var slot = _session_remote_peer_keys.find(takeover_key)
		...
		_session_remote_peer_keys.append(peer_key)

	_connection_by_peer_key[peer_key] = {"transport": transport, "peer": peer, "peer_key": peer_key}
	_peer_key_by_connection[_connection_lookup_key(transport, peer)] = peer_key
	_remember_host_remote_id(peer_key)
	_refresh_player_connection(peer_key)
	return true


func _connection_lookup_key(transport: Node, peer) -> String:
	return str(transport.get_instance_id()) + ":" + str(peer)


func _refresh_player_connection(peer_key: String) -> void:
	var slot_manager = _get_slot_manager()
	if slot_manager == null or not slot_manager.has_method("get_player_index_for_steam_id"):
		return
	var player_index = int(slot_manager.get_player_index_for_steam_id(peer_key))
	if player_index >= 0 and _connection_by_peer_key.has(peer_key):
		_connections[player_index] = _connection_by_peer_key[peer_key]


func _remove_player_connection(peer_key: String) -> void:
	for player_index in _connections.keys():
		var connection = _connections[player_index]
		if typeof(connection) == TYPE_DICTIONARY and str(connection.get("peer_key", "")) == peer_key:
			_connections.erase(player_index)


func _rebind_remote_slot(old_peer_key: String, new_peer_key: String) -> void:
	var slot_manager = _get_slot_manager()
	if slot_manager != null and slot_manager.has_method("rebind_remote_peer_key"):
		slot_manager.rebind_remote_peer_key(old_peer_key, new_peer_key)


func send_to_player(player_index: int, data: PoolByteArray, channel: int = P2P_CHANNEL_MENU, reliable: bool = true) -> bool:
	if not _connections.has(player_index):
		return false
	var connection = _connections[player_index]
	return bool(connection.transport.send_packet(connection.peer, data, channel, reliable))


func get_player_index_for_peer(peer_key: String) -> int:
	var slot_manager = _get_slot_manager()
	if slot_manager != null and slot_manager.has_method("get_player_index_for_steam_id"):
		return int(slot_manager.get_player_index_for_steam_id(peer_key))
	return -1


func send_message_to_host(message: Dictionary, reliable: bool = true) -> bool:
	if not _session_active or _is_game_host():
		return false
	return _send_p2p_json(_get_game_host_steam_id(), message, reliable)


func broadcast_message(message: Dictionary, except_peer_key: String = "", reliable: bool = true) -> bool:
	if not _session_active or not _is_game_host():
		return false
	var sent_any = false
	for peer_key_value in _get_remote_ids_for_host_sync():
		var peer_key = str(peer_key_value)
		if peer_key == "" or peer_key == except_peer_key:
			continue
		sent_any = _send_p2p_json(peer_key, message, reliable) or sent_any
	return sent_any
...
func get_lobby_state() -> String:
	if _is_host_at_character_selection_for_lobby() or _is_in_official_coop_resume_scene():
...
func _publish_session_metadata() -> void:
	if not _is_game_host() or _steam == null or _lobby_id == 0:
		return
...
func create_lobby_and_invite(open_overlay_after_create: bool = false) -> void:
	create_session(open_overlay_after_create)


func create_session(open_steam_overlay: bool = false) -> void:
	...
	if _session_active:
		...
		_update_character_lobby_status_label_state()
		_update_continue_lobby_status_label_state()
		if open_steam_overlay and _lobby_id != 0:
			open_invite_overlay()
		return

	_reset_transient_online_state_for_new_session("create_session")
	_session_active = true
	_session_id = "session:" + str(OS.get_unix_time()) + ":" + str(OS.get_ticks_usec())
	_online_role = "host"
	_is_lobby_owner = false
	if _self_steam_id.begins_with("lan:"):
		_self_steam_id = ""
	_update_self_steam_id()
	if _self_steam_id == "" or _self_steam_id == "0":
		_self_steam_id = "lan:host"
	...
	var steam_ok = false
	_open_invite_overlay_after_create = open_steam_overlay
	if _steam_transport != null and _steam_transport.is_available():
		...
		steam_ok = int(_steam_transport.start_host(lobby_type, MAX_LOBBY_MEMBERS)) == OK
	elif _ensure_steam_ready() and _steam_has_method("createLobby"):
		var fallback_lobby_type = _get_steam_const("LOBBY_TYPE_PUBLIC", LOBBY_TYPE_PUBLIC_FALLBACK) if _get_public_lobby_enabled() else _get_steam_const("LOBBY_TYPE_FRIENDS_ONLY", LOBBY_TYPE_FRIENDS_ONLY_FALLBACK)
		_steam.createLobby(fallback_lobby_type, MAX_LOBBY_MEMBERS)
		steam_ok = true

	if not lan_ok and not steam_ok:
	...
	_lobby_toggle_pending_create = steam_ok
	_lobby_toggle_close_after_create = false
	_last_lobby_create_failed_result = 0
	_update_lobby_toggle_button_state()
	_update_character_invite_button_state()
...
func join_steam_lobby(lobby_id) -> void:
	if lobby_id == null:
		_show_join_failure(_ui_text("join_failed_invalid"))
		return
	...
	if _steam_transport != null:
		if int(_steam_transport.join_lobby(target_lobby_id)) != OK:
...
func leave_lobby() -> void:
	...
	_session_active = false
	if _lan_transport != null and _lan_transport.has_method("stop"):
		_lan_transport.stop()
	if _lan_discovery != null:
		if _lan_discovery.has_method("stop_host_responder"):
			_lan_discovery.stop_host_responder()
		if _lan_discovery.has_method("stop_search"):
	...
	_peer_key_by_connection.clear()
	_connections.clear()
	_session_remote_peer_keys.clear()
	_rejected_peer_keys.clear()
	_is_lobby_owner = false
	_online_role = "none"
	_game_host_steam_id = ""
	_update_online_session_runtime_flag()
...
func _on_lobby_created(connect_result = 0, lobby_id = 0) -> void:
	...
	_session_active = true
	if _session_id == "":
		_session_id = "steam:" + str(lobby_id)
	_broadcast_transport_session_id()
	_game_host_steam_id = _self_steam_id
	_unlock_online_run_slots()
	_online_flow_started = true
	_online_flow_left_since_msec = 0
...
func _on_lobby_joined(lobby_id = 0, permissions = 0, locked = false, response = 0) -> void:
	...
	if _steam != null and _steam_has_method("getLobbyData"):
		var advertised_session_id = str(_steam.getLobbyData(lobby_id, "session_id"))
		if advertised_session_id != "":
			_session_id = advertised_session_id
	_pending_join_lobby_id = 0
	_online_flow_left_since_msec = 0
	_update_self_steam_id()
	...
	_update_online_session_runtime_flag()
	if not _is_game_host():
		var host_connection_id = _get_game_host_steam_id()
		if host_connection_id != "" and _steam_transport != null:
			_connection_by_peer_key[host_connection_id] = {"transport": _steam_transport, "peer": host_connection_id, "peer_key": host_connection_id}
			_peer_key_by_connection[_connection_lookup_key(_steam_transport, host_connection_id)] = host_connection_id
		_send_client_hello_to_host()
...
func _refresh_lobby_members(force_slot_sync: bool = false) -> void:
	...
	if _is_game_host() and _steam_transport != null:
		for member_id_value in current_member_ids:
			...
			if current_id == "" or current_id == _self_steam_id:
				continue
			_register_remote_connection(current_id, _steam_transport, current_id)
		var connected_keys = _connection_by_peer_key.keys()
		for connected_key_value in connected_keys:
			var connected_key = str(connected_key_value)
			var connection = _connection_by_peer_key.get(connected_key, {})
			if typeof(connection) == TYPE_DICTIONARY and connection.get("transport", null) == _steam_transport and not current_member_ids.has(connected_key):
				_connection_by_peer_key.erase(connected_key)
				_peer_key_by_connection.erase(_connection_lookup_key(_steam_transport, connected_key))
				_remove_player_connection(connected_key)
...
func _get_remote_ids_for_host_sync() -> Array:
	# These are opaque session peer keys. Disconnected keys deliberately remain so
	# an in-run player slot can be taken over by the next Steam or LAN connection.
	return _session_remote_peer_keys.duplicate()
...
func _prune_host_known_remote_ids_to_current_members(current_remote_ids: Array) -> void:
	for i in range(_host_known_remote_ids.size() - 1, -1, -1):
		...
		if known_id == "" or known_id == _self_steam_id or not _session_remote_peer_keys.has(known_id):
...
func _is_current_lobby_remote_member(steam_id: String) -> bool:
	...
	if _rejected_peer_keys.has(steam_id):
		return false
	if _session_remote_peer_keys.has(steam_id) or _connection_by_peer_key.has(steam_id):
		return true
...
func _ensure_host_coop_slot_for_remote(steam_id: String) -> bool:
	...
	if not _is_current_lobby_remote_member(steam_id):
		...
		return false
	if not _session_remote_peer_keys.has(steam_id) and _steam_transport != null:
		if not _register_remote_connection(steam_id, _steam_transport, steam_id):
			return false
...
func _update_self_steam_id() -> void:
	if _session_active and _online_role == "client" and _self_steam_id.begins_with("lan:"):
		return
...
func _run_steam_callbacks() -> void:
	if _steam_transport != null:
		_steam_transport.poll_callbacks()
		return
	if _steam == null:
		return
...
func _annotate_online_session_message(message: Dictionary) -> Dictionary:
	...
	var wire_session_id = _get_wire_session_id()
	if wire_session_id != "":
		wire["session_lobby_id"] = wire_session_id
	if _lobby_id != 0:
...
func _should_drop_p2p_message_for_session(from_steam_id: String, message: Dictionary) -> bool:
	...
	var expected_session_id = _get_wire_session_id()
	if not _session_active or expected_session_id == "":
		return true

	var message_lobby_id = _get_message_lobby_id(message)
	if message_lobby_id == "":
		return true
	if message_lobby_id != expected_session_id:
		return true
...
func _close_tracked_p2p_sessions() -> void:
	...
	for steam_id in ids:
		if steam_id == "" or steam_id == "0" or steam_id == _self_steam_id or steam_id.begins_with("lan:"):
			continue
...
func _poll_client_hello_retry() -> void:
	if not _session_active or _is_game_host() or not _client_hello_retry_active:
		return
	var now = OS.get_ticks_msec()
...
func _receive_steam_messages_on_channel(channel: int, limit: int) -> Array:
	if _steam_transport != null:
		return _steam_transport.receive_packets(channel, limit)
...
func _update_client_members_from_selection_state(message: Dictionary) -> void:
	var players = message.get("players", [])
	...
	for player_data in players:
		if typeof(player_data) != TYPE_DICTIONARY:
			continue
		var peer_key = str(player_data.get("steam_id", ""))
		if peer_key == "" and int(player_data.get("player_index", -1)) == 0:
			peer_key = _get_game_host_steam_id()
		if peer_key == "":
			continue
		next_members.append({"steam_id": peer_key, "name": peer_key, "owner": peer_key == _get_game_host_steam_id()})
	if not next_members.empty():
...
func _handle_p2p_message(from_steam_id: String, message: Dictionary) -> void:
	...
	if msg_type == "selection_state":
		var menu_sync_client = _get_menu_sync_manager()
		if not _is_game_host():
			_update_client_members_from_selection_state(message)
...
func _poll_and_send_host_battle_terminal_state() -> void:
	if not _is_game_host() or not _session_active or not _is_in_game_scene():
		return
...
func _prepare_steam_messages_session_with_peer(steam_id: String, reason: String = "") -> void:
	var connection = _connection_by_peer_key.get(steam_id, {})
	if typeof(connection) == TYPE_DICTIONARY and connection.get("transport", null) != null and connection.get("transport", null) != _steam_transport:
		return
...
func _send_p2p_json(target_steam_id: String, message: Dictionary, reliable: bool = true) -> bool:
	...
	if not _connection_by_peer_key.has(target_steam_id) and not _ensure_steam_ready():
		return false
...
func _receive_p2p_json_chunk(from_steam_id: String, message: Dictionary) -> Dictionary:
	var message_lobby_id = _get_message_lobby_id(message)
	var expected_session_id = _get_wire_session_id()
	if expected_session_id != "" and message_lobby_id != expected_session_id:
		return {}
...
func _poll_online_flow_lifecycle() -> void:
	if not _session_active or not _online_flow_started:
		return
...
func _has_active_online_session() -> bool:
	return _session_active or _pending_lan_join or _pending_join_lobby_id != 0 or _client_join_requested_lobby_id != 0
...
func _should_show_joining_overlay() -> bool:
	...
	if _pending_join_lobby_id != 0 or _client_join_requested_lobby_id != 0:
		return true
	if _online_role == "client" and _session_active:
...
func _should_show_character_lobby_status_label() -> bool:
	...
	return _main_menu_online_start_pending or _lobby_toggle_pending_create or _session_active or _online_role != "none" or _last_lobby_create_failed_result != 0


func _get_lobby_status_text() -> String:
	var creating = _lobby_toggle_pending_create and not _lobby_toggle_close_after_create
	if creating and not _session_active:
		return _ui_text("status_creating_steam")

	if _main_menu_online_start_pending and not _session_active:
...
func _update_character_invite_button_state() -> void:
	...
	elif _session_active and _lobby_id == 0:
		_character_invite_button.text = _ui_text("lan_room_active")
		_character_invite_button.disabled = true
	else:
...
func _update_continue_invite_button_state() -> void:
	...
	if creating and not _session_active:
		...
		_continue_invite_button.text = _ui_text("lan_room_active")
		_continue_invite_button.disabled = true
...
func _update_lobby_toggle_button_state() -> void:
	...
	_lobby_toggle_button.pressed = (_session_active or _lobby_toggle_pending_create) and not _lobby_toggle_close_after_create
...
func _poll_official_continue_auto_lobby() -> void:
	if not AUTO_CREATE_LOBBY_ON_OFFICIAL_COOP_CONTINUE:
		return
	if _session_active or _lobby_toggle_pending_create or _online_role == "client":
		_official_continue_auto_lobby_armed = false
		_official_continue_auto_lobby_first_seen_msec = 0
		return
	if not _is_host_in_official_coop_continue_resume_scene():
